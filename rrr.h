...
typedef (*rrr_painter_job_t)(void);
typedef (*rrr_painter_run_proc_t)(void *data, rrr_painter_job_t job);
...
RRR_API rrr_rect_t rrr_canvas_get_normalized_rect(rrr_canvas_t canvas, rrr_rect_t rect) {
    ...
    if (rect.x >= canvas.image.width || rect.y >= canvas.image.height) {
        return RRR_RECT_ZERO;
    }

    if (rect.x + rect.width >= canvas.image.width) {
        result.width = canvas.image.width - rect.x;
    }

    if (rect.y + rect.height >= canvas.image.height) {
        result.height = canvas.image.height - rect.y;
    }
...
RRR_API void rrr_canvas_draw_rect(rrr_canvas_t canvas, rrr_rect_t rect, rrr_color_t color) {
    ...
    for (rrr_usize_t y = draw_rect.y; y <= draw_rect.height; y++) {
        for (rrr_usize_t x = draw_rect.x; x < draw_rect.width; x++) {
            RRR_IMAGE_PIXEL(canvas.image, x, y) = color;
        }
    }
}
